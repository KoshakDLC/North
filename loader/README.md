...
Десктопный лоадер клиента: проверяет ключ, кладёт свежую сборку в `mods` и открывает игру.
...
1. Проверяет лицензионный ключ.
2. Ищет папку Minecraft.
3. Проверяет, установлен ли Fabric для 1.21.8.
4. Скачивает свежую сборку клиента из релизов GitHub.
5. Копирует джарник в `mods`, удаляя прошлые сборки `wild-*.jar`, и запускает лаунчер.
...
Пока поле «Сервер лицензий» пустое, загрузчик работает в офлайн-режиме и принимает любой
непустой ключ. Если указать URL, на него уйдёт запрос:

```
POST {url}
{"key":"XXXX-XXXX","hwid":"9670530ce9b89130"}

200 OK
{"ok":true,"user":"lowfi"}
```

Ключ считается принятым при HTTP 2xx и `"ok":true` в теле. Поле `user` необязательное —
если оно есть, имя покажется в журнале. HWID считается как SHA-256 от имени пользователя,
имени машины и архитектуры ОС.
...
| Ключ | Смысл |
| --- | --- |
| `license.key` | последний введённый ключ |
| `license.api` | адрес сервера лицензий, пусто — офлайн |
| `minecraft.dir` | папка игры, пусто — `%APPDATA%\.minecraft` |
