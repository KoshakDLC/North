...
public enum KeyboardKey {
   ...
   private static final Map<Integer, KeyboardKey> VALUES_BY_KEY = new HashMap<>();
   private static final Map<String, KeyboardKey> VALUES_BY_KEY_2 = new HashMap<>();
   ...
   public static String resolve(int i) {
      if (i == -200) {
         ...
         KeyboardKey keyboardKey = VALUES_BY_KEY.getOrDefault(i, KEY_NONE);
         return keyboardKey.text;
      }
   }

   public static int compute(String string) {
      KeyboardKey keyboardKey2 = VALUES_BY_KEY_2.getOrDefault(string.toLowerCase(), KEY_NONE);
      return keyboardKey2.intValue;
   }

   public static KeyboardKey resolve2(int i) {
      return VALUES_BY_KEY.getOrDefault(i, KEY_NONE);
   }

   public boolean check(int i) {
      return i == this.getIntValue();
   }
   ...
   @Generated
   private KeyboardKey(String string2, int j) {
      this.text = string2;
      this.intValue = j;
   }

   static {
      for (KeyboardKey keyboardKey3 : values()) {
         VALUES_BY_KEY.put(keyboardKey3.intValue, keyboardKey3);
         VALUES_BY_KEY_2.put(keyboardKey3.text.toLowerCase(), keyboardKey3);
      }
   }
}
