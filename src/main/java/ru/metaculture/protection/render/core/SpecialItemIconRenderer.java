...
public class SpecialItemIconRenderer {
   private static final Map<String, ItemStack> VALUES_BY_KEY = new HashMap<>();

   public static void invoke(DrawContext drawContext, String string, float f, float g) {
      ItemStack itemStack = resolve(string);
      if (itemStack != null && !itemStack.isEmpty()) {
         drawContext.drawItem(itemStack, (int)f, (int)g);
      }
   }

   public static ItemStack resolve(String string) {
      if (VALUES_BY_KEY.containsKey(string)) {
         return VALUES_BY_KEY.get(string);
      } else if (HolyWorldItemParser.check2(string)) {
         ItemStack itemStack2 = HolyWorldItemParser.resolve4(string);
         if (itemStack2.isEmpty()) {
            return itemStack2;
         } else {
            VALUES_BY_KEY.put(string, itemStack2);
            return itemStack2;
         }
      } else {
         ItemStack itemStack3 = switch (string) {
            case "Сфера Хаоса" -> resolve2(
         ...
         VALUES_BY_KEY.put(string, itemStack3);
         return itemStack3;
      }
   }

   private static ItemStack resolve2(String string) {
      ItemStack itemStack4 = new ItemStack(Items.PLAYER_HEAD);
      ...
      itemStack4.set(DataComponentTypes.PROFILE, new ProfileComponent(gameProfile));
      return itemStack4;
   }
}
